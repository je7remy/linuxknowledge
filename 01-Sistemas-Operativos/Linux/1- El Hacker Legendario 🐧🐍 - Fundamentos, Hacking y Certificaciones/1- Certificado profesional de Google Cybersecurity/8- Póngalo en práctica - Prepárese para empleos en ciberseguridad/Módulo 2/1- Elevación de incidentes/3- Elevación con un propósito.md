---
tipo: teoria
tags: [google-cybersecurity, el-hacker-legendario, modulo-2]
actualizado: 2026-05-28
---

# Elevación con un propósito

Anteriormente aprendió sobre la Escalada de incidentes de Seguridad y las habilidades necesarias para ayudarle a escalar incidentes. En esta lectura, aprenderá la importancia de escalar los problemas de Seguridad y el Impacto potencial de no escalar un Problema.

## Escalado de incidentes

El Escalado de incidentes de Seguridad es el proceso de identificación de un potencial incidente de Seguridad. Durante este proceso, los incidentes potenciales se transfieren a un departamento o a un miembro del equipo con más experiencia. Como analista de Seguridad, se espera que reconozca los problemas potenciales, como cuando un empleado introduce excesivamente credenciales erróneas en su cuenta, y lo informe a la persona adecuada. Cuando se incorpore a una nueva organización, conocerá los procesos y procedimientos específicos para la escalada de incidentes.

## Notificación de violaciones

Muchos países tienen leyes de notificación de violaciones, por lo que es importante que se familiarice con las leyes aplicables en la zona en la que opera su empresa. Las leyes de notificación de violaciones exigen que las empresas y las entidades gubernamentales notifiquen a los particulares las violaciones de seguridad que afecten a información de identificación personal (PII). La PII incluye números de identificación personal (por ejemplo, números de la Seguridad Social, números del carné de conducir, etc.), historiales médicos, direcciones y otra información sensible de los clientes. Como analista de seguridad de nivel básico, tendrá que estar al tanto de las diversas leyes de seguridad, sobre todo porque se actualizan con regularidad.

## Problemas de Seguridad de bajo nivel

Los problemas de seguridad de bajo nivel son riesgos de seguridad que no dan lugar a la Exposición de PII. Estos problemas pueden incluir los siguientes y otros riesgos:

- Que un empleado tenga un intento fallido de inicio de sesión en su cuenta
    
- Un empleado que descarga software no aprobado en su portátil de trabajo
    

Estos problemas no son retos importantes para la Seguridad, pero deben investigarse más a fondo en caso de que sea necesario escalarlos. Un empleado que teclea una contraseña dos o tres veces puede no ser motivo de preocupación. Pero si ese empleado teclea una contraseña 15 veces en 30 minutos, podría haber un Problema que necesita ser escalado. ¿Y si los múltiples intentos fallidos de inicio de sesión fueran de un actor malicioso que intenta comprometer la cuenta de un empleado? ¿Y si un empleado se descarga un juego de Internet o un software en su portátil de trabajo que está infectado con software malicioso? Usted aprendió previamente que el software malicioso es un software diseñado para dañar dispositivos o redes. Si se descarga software malicioso en la red de una organización, puede provocar pérdidas financieras e incluso pérdida de reputación ante los clientes de la organización. Aunque los problemas de seguridad de bajo nivel no se consideran amenazas de seguridad importantes, deben investigarse para garantizar que tienen un impacto mínimo en la organización.

## El proceso de Elevación de Casos

Cada empresa tiene protocolos y procedimientos diferentes, incluidas políticas de escalado únicas. Estas políticas detallan a quién se debe notificar cuando se recibe una alerta de Seguridad y con quién se debe contactar si el primero en responder no está disponible. La política también determinará cómo alguien debe escalar específicamente un incidente, ya sea a través del escritorio de TI, de una herramienta de gestión de incidentes o de la comunicación directa entre los miembros del Equipo de Seguridad.

## Puntos clave

El Escalado de incidentes es esencial para proteger los Datos de una organización. Cada organización puede tener una forma diferente de escalar los incidentes de Seguridad. Un analista de Seguridad debe ser consciente de los protocolos de Elevación que existen en su organización. Tanto los pequeños como los grandes problemas de Seguridad deben ser escalados al equipo o miembro del equipo apropiado.


---


A los analistas de seguridad se les contrata para proteger los datos y activos de la empresa, lo que incluye saber cuándo y cómo escalar los incidentes de seguridad. En este vídeo, definiremos el **Escalado de incidentes de seguridad** y hablaremos de su función a la hora de tomar decisiones que ayuden a proteger los datos y recursos de su organización.

### ¿Qué es el Escalado de incidentes?

¿Y por qué es tan importante para los profesionales de la Seguridad?

La **Elevación de incidentes** es el proceso de identificar un posible incidente de seguridad, triarlo y (si procede) pasarlo a un miembro del Equipo con más experiencia. También es importante reconocer que no todos los incidentes necesitan ser escalados. En este vídeo, cubriremos qué tipos de incidentes deberían ser escalados.

Como analista de nivel básico, es poco probable que responda a incidentes de Seguridad de forma independiente. Sin embargo, es importante que sepa cómo evaluar y remitir los incidentes al individuo o equipo adecuado cuando sea necesario.

### Habilidades Esenciales para la Escalada

Discutiremos las habilidades esenciales necesarias para remitir adecuadamente los incidentes de seguridad. Hay dos habilidades esenciales que le ayudarán a identificar los incidentes de seguridad que necesitan ser remitidos:

- La **atención a los detalles**.
    
- La capacidad de **seguir las directrices** o procesos de remisión de una organización.
    

La atención a los detalles le ayudará a identificar rápidamente cuando algo no parece ir bien en la red o los sistemas de la organización. Seguir las directrices o procesos de escalado de una empresa le ayudará a saber cómo escalar adecuadamente el problema que ha identificado.

### Roles y Responsabilidades

Los equipos de seguridad de las organizaciones más grandes tienen muchos niveles, y cada nivel, o miembro, de ese equipo desempeña un papel importante en la protección de los recursos y datos de la empresa. Sin embargo, las empresas más pequeñas y medianas tienen sólo una o dos personas responsables de la seguridad de la organización.

Por ahora, nos centraremos en las funciones en las organizaciones más grandes. Desde el Director de Seguridad de la Información (también conocido como el CISO), hasta el equipo de ingeniería, el equipo de relaciones públicas e incluso el equipo jurídico, cada miembro del equipo de seguridad importa. La función de cada miembro del Equipo depende de la naturaleza y el Alcance del Incidente. Estas funciones se destacan dentro de un proceso de escalada de la empresa.

### La Importancia de la Rapidez

Incluso el incidente de Seguridad más pequeño puede convertirse en un Problema mucho mayor si no se aborda. ¡Y ahí es donde entra usted!

Imagínese que está trabajando en su escritorio y observa lo que parece ser un incidente menor, pero decide tomarse un descanso antes de abordarlo o escalarlo. Esta decisión podría tener consecuencias importantes. Si un pequeño problema no se resuelve durante demasiado tiempo, puede convertirse en un problema mayor que **cueste dinero a la empresa**, **exponga datos confidenciales de los clientes**, o **dañe la reputación de la empresa**.

Sin embargo, con un alto nivel de atención a los detalles y la capacidad de seguir las directrices y procesos de escalado de su organización, puede ser posible evitar exponer a la empresa, y a sus clientes, a incidentes perjudiciales.

### Su Papel como Analista Básico

Como analista de nivel básico, usted desempeña una función importante. Usted ayuda al equipo de seguridad a identificar problemas dentro de la red y los sistemas y ayuda a asegurarse de que la persona adecuada del equipo es alertada cuando se producen incidentes.

> Piense en una cadena de montaje. ¿El paso final de la cadena se vería afectado negativamente si el primer paso se realizara de forma incorrecta, o no se realizara en absoluto? ¡Por supuesto que sí!

Cada decisión que tome ayudará a todo el equipo de seguridad a proteger los activos y datos de una organización. Saber cuándo y cómo escalar los incidentes de seguridad es una de las muchas decisiones importantes que tendrá que tomar a diario.

Más adelante en este curso, hablaremos de los distintos niveles de incidentes de seguridad. Conocer esos niveles le ayudará a determinar el nivel de urgencia necesario para escalar los distintos tipos de incidentes.

---

## Navegación

- ⬆️ Carpeta: [[_8- Póngalo en práctica - Prepárese para empleos en ciberseguridad|8- Póngalo en práctica - Prepárese para empleos en ciberseguridad]]
- ⬅️ Anterior: [[2- La importancia de la Elevación]]
- ➡️ Siguiente: [[4- Prepárese para la escalada mediante el reconocimiento de la Seguridad]]
